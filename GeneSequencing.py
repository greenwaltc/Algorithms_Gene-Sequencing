...
###################################################################################################					
	...
	def alignNoBand(self, seq1, seq2):
		...
		Table = [[{"cost": 0, "back_ptr": []} for i in range(len_sequence1)] for j in range(len_sequence2)]
		...
		for i in range(1, len_sequence2):
			for j in range(1, len_sequence1):
				...
				for k in range(len(minCosts)):
					if minCosts[k] == LEFT:
						Table[i][j]["back_ptr"].append((i-1, j))
					elif minCosts[k] == UPPER:
						Table[i][j]["back_ptr"].append((i, j-1))
					elif minCosts[k] == DIAG:
						Table[i][j]["back_ptr"].append((i-1, j-1))

		return Table[len_sequence2-1][len_sequence1-1]["cost"]
	...
	def minNeighborCosts(self, i, j, seq1, seq2, Table):
		...
		costs[0] = Table[i-1][j]["cost"] + 5  # Cost of left neighbor (cost of indel)  # TODO fixme
		costs[1] = Table[i][j-1]["cost"] + 5  # Cost of upper neighbor (cost of indel)
		costs[2] = Table[i-1][j-1]["cost"] + self.diff(i, j, seq1, seq2, Table)  # Cost of diag neighbor (-3 if char match, else 1)
	...
	def diff(self, i, j, seq1, seq2, Table):
		if seq1[j] == seq2[i]:
			return -3
		return 1